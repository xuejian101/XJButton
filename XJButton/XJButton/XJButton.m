 ...
 *  按钮类型
 */
@property (nonatomic,assign) XJImageButtonType type;
 ...
 */
- (instancetype)initWithType:(XJImageButtonType)type AndMarginArr:(NSArray *)arr{
    self = [super init];
    if (self) {
        _marginArr = arr;
        _type = type;
...
- (void)setRootSubView{
    ...
    switch (_marginArr.count) {
        case 0:
        {
            _marginMiddle = XJImageButtonDefaultMargin;
            _marginTopOrLeft = XJImageButtonDefaultUnSetMargin;
            _marginBottomOrRight = XJImageButtonDefaultUnSetMargin;
            break;
        }
        case 1:
        {
...
- (void)setLeft{
    [_topOrLeftView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        if (self.marginTopOrLeft != XJImageButtonDefaultUnSetMargin) {
    ...
    [_bottomOrRightView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        if (self.marginBottomOrRight != XJImageButtonDefaultUnSetMargin) {
...
- (void)updateFrame{
    switch (_type) {
        case XJImageButtonTypeLeft:
        {
            [self setLeft];
            break;
        }
        case XJImageButtonTypeRight:
        {
            [self setRight];
            break;
        }
        case XJImageButtonTypeTop:
        {
            [self setTop];
            break;
        }
        case XJImageButtonTypeBottom:
        {
            [self setBottom];
            break;
        }
...
- (void)setNormolBackgroundColor:(UIColor *)normalColor
    ...
    if (_currentState == XJButtonStateDisable) {
        self.backgroundColor = disableColor;
    }else if(_currentState == XJButtonStateNormal){
        self.backgroundColor = normalColor;
    }else if(_currentState == XJButtonStateSelected){
        self.backgroundColor = selectedColor;
    }else if(_currentState == XJButtonStateHighLight){
...
- (void)setNormal{
    ...
    if (_currentState == XJButtonStateNormal) {
        return;
    }
    _currentState = XJButtonStateNormal;
    if (_normalColor) {
...
- (void)setHighLight{
    ...
    if (_currentState == XJButtonStateHighLight) {
        return;
    }
    _currentState = XJButtonStateHighLight;
    if (_highLightColor) {
...
-(void)setSelected{
    if (_currentState == XJButtonStateSelected) {
        return;
    }
    _currentState = XJButtonStateSelected;
    if (_selectedColor) {
...
- (void)setDisable{
    ...
    _currentState = XJButtonStateDisable;
    if (_disableColor) {
